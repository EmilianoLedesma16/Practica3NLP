...
# Mapeo de etiquetas POS de spaCy a WordNet
pos_spacy_to_wordnet = {
    'NOUN': 'n',
    'VERB': 'v',
    'ADJ': 'a',
    'ADV': 'r'
...
def lematizar_con_respaldo(token):
    # Intentar lematización con spaCy
    lema_spacy = token.lemma_.lower()

    # Si spaCy no cambió nada o si el token es plural, usamos NLTK con POS mapeado
    if lema_spacy == token.text.lower() or token.tag_ == "NNS":
        pos_nltk = pos_spacy_to_wordnet.get(token.pos_, 'n')  # Usar sustantivo como fallback
        lema_nltk = lemmatizer.lemmatize(token.text.lower(), pos=pos_nltk)
        return lema_nltk
...
def procesar_corpus(ruta_entrada, ruta_salida):
    try:
        ...
        if 'Title' not in df.columns or 'Abstract' not in df.columns or 'Section' not in df.columns:
            raise ValueError("El archivo de entrada debe contener las columnas 'Title', 'Abstract' y 'Section'.")

        # concatenar título y resumen
        df['Concatenated_text'] = df['Title'].fillna('') + ". " + df['Abstract'].fillna('')

        # Aplicar la normalización al texto concatenado
        df['Normalized_text'] = df['Concatenated_text'].apply(normalizar_texto)
        
        # movemos 'Section' al final del DataFrame
...
if __name__ == "__main__":
    ...
    archivos = [
        ("arxiv_raw_corpus.csv", "arxiv_normalized_corpus.csv")
    ]
